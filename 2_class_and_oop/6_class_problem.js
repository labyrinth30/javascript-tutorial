...
const cIveMembers = iveMembers.map(e => new FemaleIdol(e.name,e.year));

console.log(cIveMembers);

const cBtsMembers = btsMembers.map((e) => new MaleIdol(e['name'], e['year']));
...
const korea2 = new Country(
    '한국',
    [ new IdolGroup(
        '아이브',
        [ iveMembers.map(e => new Idol(e.name, e.year))]
    ),
    new IdolGroup(
        'BTS',
        [ btsMembers.map(e => new Idol(e.name, e.year))]
    ),
],
);
